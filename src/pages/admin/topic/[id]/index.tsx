	...
	return (
			<div>
				...
				<form onSubmit={onSubmit}>
					...
					<input type="text" name="introduction_text" id="introduction_text" defaultValue={currentTopic?.introduction_text} disabled={!isModify}/>
					...
					{themesList && themesList.length > 0 && currentTopicTheme ? (
							<select name="theme" id="theme" disabled={!isModify}>
								{themesList?.map((theme, index) => {
									return (
											<option key={index}  value={theme.id} selected={theme.id === currentTopicTheme.id}>{theme.slug}</option>
									)
								})}
							</select>
					) : null}
