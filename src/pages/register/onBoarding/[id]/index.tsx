...
import {Article_Frequency, Format, Format_Name, Theme, Theme_Name} from "@prisma/client";
...
export default function OnBoarding(){
	...
	useEffect(() => {
		getFormats().then((formats) => setFormats(formats));
				...
				<div className={styles.sectionConnexion}>
					...
					<div className={styles.containerConnexion} >
						...
						<div className={`${styles.contentConnexion} ${styles.flexColumn}`}>
							{themes.map((theme: Theme, index) => (
									<div key={theme.id} className={styles.checkboxThemes}>
										<input data-theme-id={theme.id} name={theme.slug} className="themeCheckbox" key={index} type="checkbox" onChange={()=>handleChangeTheme(theme.id)}/>
						...
						<div className={styles.contentConnexion}>
							{formats.map((format: Format, index) => (
									<div key={format.id} className={styles.checkboxFormats}>
										<input data-format-id={format.id} name={format.slug} className="formatCheckbox" key={index} type="checkbox" onChange={()=>handleChangeFormat(format.id)}/>
										<label>{format.slug}</label>
									</div>
							))}
						</div>
						<div className={styles.contentConnexion}>
							<input id='numberArticle' type="number" min="0" onChange={(event) => {setNumberArticle(event.target.value)}} />
							<select name="frequency" id="frequency" className={styles.selection} multiple onChange={(event) => {setSelectedArticleFrequencies(event.target.value)}} >
						...
						<div className={`${styles.contentConnexion} ${styles.connexionLinks}`}>
							<button onClick={() => onSignUp()} className={styles.registerButton}>S'inscrire</button>
						</div>
					</div>
				</div>
			</div>
